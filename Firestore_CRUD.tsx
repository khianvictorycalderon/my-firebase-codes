...
const fs_write = async (path: string, data: Data, isMerge: boolean): Promise<void> => {
  const docRef = doc(firestore_db, path);
  await setDoc(docRef, data, { merge: isMerge });
};

// Read Data On Request only (Specify the field)
const fs_read = async (path: string, field: string): Promise<any> => {
  const docRef = doc(firestore_db, path);
...
const fs_reading = (path: string, field: string, callback?: (value: any) => void): (() => void) => {
  const docRef = doc(firestore_db, path);
...
// Delete Data
const fs_delete = async (path: string): Promise<void> => {
  try {
    const docRef = doc(firestore_db, path);
